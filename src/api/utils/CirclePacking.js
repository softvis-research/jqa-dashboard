...
class CirclePackingHelper {
        ...
        var stratify = d3
            ...
            .parentId(function(d) {
                if (d.name.lastIndexOf(".") !== -1) {
                    // classes and subpackes
                    return d.name.substring(0, d.name.lastIndexOf("."));
                } else if (d.name !== projectName) {
                    // a root package
                    return projectName;
                } else {
    ...
    normalizeHotspots(hierarchicalData) {
        for (var i = 0; i < hierarchicalData.children.length; i++) {
            ...
            hierarchicalData.children[i].name = hierarchicalData.children[
                i
            ].data.name.substring(lastDot + 1);
            hierarchicalData.children[i].loc =
                hierarchicalData.children[i].data.loc;
            hierarchicalData.children[i].complexity =
                hierarchicalData.children[i].data.complexity;
    ...
    normalizeTestCoverage(hierarchicalData) {
        for (var i = 0; i < hierarchicalData.children.length; i++) {
            hierarchicalData.children[i].key =
                hierarchicalData.children[i].data.id;
            hierarchicalData.children[i].coverage =
                hierarchicalData.children[i].data.coverage;
            hierarchicalData.children[i].loc =
                hierarchicalData.children[i].data.loc;

            if (!hierarchicalData.children[i].data.fqn) {
                var lastDot = hierarchicalData.children[
                    i
                ...
                hierarchicalData.children[i].name =
                    hierarchicalData.children[i].data.name;
            }
